...
def run(
    ...
    # 将 LoRA 微调的权重加载到基础模型中
    # 通过 LoRA 的方式，模型只需调整少量参数即可适应特定任务，而无需重新训练整个模型
    if use_lora:
        ...
        for i, line in enumerate(lines):
            ...
            if (" sum(accs)" in line) & (lines[i - 1].startswith("====GR====")):
                ...
                if q not in q_list:
                    q_list.append(q)
    start_time = time.time()
    # 根据任务类型加载不同的数据集（如 bamboogle、fever、svamp 等）
    if args.task in [
        "game24",
        "bamboogle",
        "2wiki",
        ...
        if args.task == "game24":
            data = list(pd.read_csv("tot/data/24/24.csv")["Puzzles"])[
                args.task_start_index : args.task_end_index
            ]
        elif args.task == "bamboogle":
            if args.train == False:
                # 问题列表
                data = list(
                    pd.read_csv("tot/data/bamboogle/Bamboogle Prerelease - Sheet1.csv")[
                        "Question"
                    ]
                )
                # 答案列表
        ...
        file_out = open(args.data_json_file, "a")  # 输出文件
        ...
        # 这段代码是一个典型的机器学习评估循环，用于测试模型在各种任务上的表现，并记录详细的评估结果。
        # 它首先将模型设置为评估模式，然后遍历数据集中的每个样本。对于每个样本，它使用模型进行推理，并根据任务类型对推理结果进行处理。
        # 然后，它记录每个样本的评估结果，并计算平均准确率和至少有一个正确答案的准确率。最后，它将评估结果写入JSON文件中。
        # 这段代码的主要目的是评估模型在各种任务上的表现，并记录详细的评估结果。
        # 它可以帮助开发人员了解模型的性能，并根据需要进行调整和改进。
        for d in data:
            # 推理模式选择
            if args.naive_run:
            ...
            for d_idx, d_i in enumerate(d):
                ...
                if len(accs) == 0:
                    ...
                    continue
                # 计算准确率指标
                cnt_avg += sum(accs) / len(accs)  # 计算平均准确率
                cnt_any += any(accs)  # 计算至少有一个正确答案的准确率
        ...
        for d in data:
            ...
            if args.naive_run:
                args.n_generate_sample = 10
                ys, info = naive_solve(
                    args, task_prompt, d["question"][0], model, tokenizer, device
                )
                out = {}
            else:
                ys, info, out = solve(
...
def parse_args():
    ...
    args.add_argument(
        "--backend",
        type=str,
        choices=["gpt-4", "gpt-3.5-turbo", "llama2-7b"],
        default="llama2-7b",
    )
    ...
    args.add_argument("--base_model", type=str, default="NousResearch/Llama-2-7b-hf")
